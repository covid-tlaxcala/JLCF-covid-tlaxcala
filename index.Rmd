...
#database_title <- paste("211012COVID19MEXICO.csv")
...
mn_Def_Mx_Edos <- mn_Def_Mx_Edos[-grep("9999-99-99", mn_Def_Mx_Edos$fecha_def),]
...
rm(mn_Mx_2, sum_pacientes, n_amb, n_hosp, n_Mx, plot_Casos_Mx, MujPosMexAmb, MujPosMexHosp, HomPosMexAmb, HomPosMexHosp, CasosSexoMex, CasosSexoMex_plot)
```
...
#Hospitalizados
d.f <- Mex_Data_COVID %>% 
  count(fecha_sintomas, tipo_paciente, Age_group, value)
...
rm(Decesos_Mx, NumFallMex, Decesos_Mx_St, Decesos_Mx_2, n_amb, n_hosp, n_Mx, plot_Casos_Mx,sum_decesos, MujDecMexAmb, MujDecMexHosp, HomDecMexAmb, HomDecMexHosp, DecesosSexoMex)
```
...
#Hospitalizados
d.f <- Mex_Data_COVID %>% 
  count(fecha_def,tipo_paciente,Age_group,value)
...
```{r Porcentaje Defunciones México,warning=FALSE, echo=FALSE, fig.align="center", eval = F}
...
CasosSexoTlax_plot <- plot_ly(CasosSexoTlax, labels = ~Tipo_paciente, values = ~Casos,
  ...
  layout(title = paste('<b>Casos por sexo en Tlaxcala de COVID-19 al</b>', today, sep = " "),
         xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
         yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE));CasosSexoTlax_plot
rm(CasosSexoTlax)
```
...
mnAct_Mx <- activos_Mx %>% 
  count(state_name)
names(mnAct_Mx) <- c("state_name", "value")
...
mnAct_Tlx <- activos_Tlx %>% 
  count(municipio_name,region,municipio_code)

names(mnAct_Tlx) <- c("municipio_name","region","municipio_code", "value")
